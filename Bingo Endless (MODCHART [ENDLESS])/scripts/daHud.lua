...
local settings = {
	...
	showNoteCombo = false, -- disable this if its annoying lmao
	noteComboX = 311,
	noteComboY = 223,
...
function onCreatePost()
	...
	makeLuaText("songName", '', 1, screenWidth/2, 15)
	setTextAlignment("songName", "center") -- change text alignment
	setTextFont("songName", dFont) -- change text font
	setTextSize("songName", 18) -- change text size

	setProperty("songName.wordWrap", false) -- toggle text wrap
	setProperty("songName.autoSize", true) -- toggle text auto size

	setProperty("songName.antialiasing", gAA) -- toggle antialiasing

	setScrollFactor("songName", 0, 0) -- set scroll factor
	setObjectCamera("songName", "hud") -- set text camera

	-- recentering the song name text
	setProperty("songName.x", screenWidth/2 - math.floor(string.len(getTextString("songName")))/4 * 18)
	...
	if (settings.hideTimeBar) == false then
		...
		makeLuaText("currentTime", "?:??", 1, 5, 15)
		...
		if getPropertyFromClass("ClientPrefs", "showFPS") == true then
			setProperty("currentTime.y", 40)
		end

		-- SONG LENGTH TEXT --
		makeLuaText("totalTime", "?:??", 1, screenWidth - 50, 15)
		...
		if not downscroll then -- if upscroll
			...
			setProperty("currentTime.y", screenHeight - 35)
			setProperty("totalTime.y", screenHeight - 35)
	...
	if (settings.coolBar) then
		...
		setProperty("healthBarBG.yAdd", -34)
...
function onSongStart()
	...
	setTextString("currentTime", "0:00")
	...
	if string.len(secondsLeft) == 1 then 
		secondsLeft = "0"..secondsLeft
	end

	setTextString("totalTime", minutesLeft..":"..secondsLeft)
...
function onUpdatePost(dt)
	...
	local accuracyHitBased = (
		(
			getHitsOfRating('sicks') + 
			getHitsOfRating('goods') + 
			getHitsOfRating('bads') + 
			getHitsOfRating('shits')
		) / (getProperty('songHits')+getProperty('songMisses'))
	) / 100
	...
	local ratingTxt = "Rating: " .. (
		...
		(
			ratingName ..
			" (" .. tostring(math.truncate(accuracyHitBased * 100, 2)) .. "%)" ..
	...
	if (settings.coolBar) then
		...
		setObjectClip("healthBarP2", vvv, 0, math.clamp(math.lerp(0, getProperty("healthBarP2.frame.frame.width"), health / 1), 1, 924), -1)
